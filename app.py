...
if st.session_state.step == 0:
    ...
    st.session_state.template_choice = st.selectbox(
        "Choose Excel Template Profile", template_options
    )
    ...
    if st.button("Proceed", type="primary"):
        if st.session_state.promo_name:
            ...
            try:
                ...
                st.error(
                    f"Could not find Excel template: '{excel_path}'. Did you put it in the templates folder?"
                )
        else:
...
# ==========================================
    ...
    # DYNAMIC SHEET SELECTION HAPPENS HERE
    st.session_state.base_sheet = st.selectbox(
        "Choose Base Sheet", sheets, index=default_idx
    )
...
# ==========================================
    ...
    st.session_state.template_handler.render_step_3(
        st.session_state.excel_mgr,
        st.session_state.configs[st.session_state.template_choice],
    )

elif st.session_state.step == 2:
    st.session_state.template_handler.render_step_4(
        st.session_state.excel_mgr,
        st.session_state.configs[st.session_state.template_choice],
    )

elif st.session_state.step == 3:
    st.session_state.template_handler.render_step_5(
        st.session_state.excel_mgr,
        st.session_state.configs[st.session_state.template_choice],
    )
