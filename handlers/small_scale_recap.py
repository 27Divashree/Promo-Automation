...
class Handler:
    ...
    def render_step_3(self, mgr, config):
        ...
        with col1:
            ...
            ty_q_end = st.date_input(
                "TY Qualify End", value=today + datetime.timedelta(days=14)
            )
            ty_r_start = st.date_input(
                "TY Redeem Start", value=today + datetime.timedelta(days=15)
            )
            ty_r_end = st.date_input(
                "TY Redeem End", value=today + datetime.timedelta(days=29)
            )
        with col2:
            ly_q_start = st.date_input(
                "LY Qualify Start", value=ty_q_start - datetime.timedelta(days=364)
            )
            ly_q_end = st.date_input(
                "LY Qualify End", value=ty_q_end - datetime.timedelta(days=364)
            )
            ly_r_start = st.date_input(
                "LY Redeem Start", value=ty_r_start - datetime.timedelta(days=364)
            )
            ly_r_end = st.date_input(
                "LY Redeem End", value=ty_r_end - datetime.timedelta(days=364)
            )

        st.divider()
        st.subheader("3.2 Article List Upload")
        uploaded_file = st.file_uploader(
            f"Upload Article CSV/Excel", type=["csv", "xlsx"]
        )
        ...
        if st.button("Generate SQL & Create Tab", type="primary"):
            ...
            mgr.write_to_cell(
                tab_name, mappings["ty_qualify_dates"], f"{ty_q_start} - {ty_q_end}"
            )
            mgr.write_to_cell(
                tab_name, mappings["ty_redeem_dates"], f"{ty_r_start} - {ty_r_end}"
            )
            mgr.write_to_cell(
                tab_name, mappings["ly_qualify_dates"], f"{ly_q_start} - {ly_q_end}"
            )
            mgr.write_to_cell(
                tab_name, mappings["ly_redeem_dates"], f"{ly_r_start} - {ly_r_end}"
            )
